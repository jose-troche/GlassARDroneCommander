...
public class MainActivity extends Activity implements
    ...
    private void sendMessage(String message) {
        ...
        if (message.length() > 0) {
            new sendDatagramTask().execute(message + "\n");
        }
    ...
    private class sendDatagramTask extends AsyncTask<String, Void, Void> {
        @Override
        protected Void doInBackground(String... msg) {
            try {
                byte[] buf = msg[0].getBytes();
                String ardroneIP = "192.168.1.225";
                InetAddress ardroneInetAddress = InetAddress.getByName(ardroneIP);
                int ardroneCommandsUdpPort = 5556;
                DatagramSocket socket = new DatagramSocket();
                DatagramPacket packet = new DatagramPacket(buf, buf.length, ardroneInetAddress, ardroneCommandsUdpPort);
                socket.send(packet);
                socket.close();
            } catch (IOException e) {
